...
vec3 calcGradientA(sampler3D volume, VOLUME_PARAMETERS volumeParameters, vec3 samplePos, float t, vec3 rayDirection, SAMPLER2D_TYPE entryPoints, TEXTURE_PARAMETERS entryParameters) {
    ...
    if (t == 0.0)
        ...
        vec3 offset = volumeParameters.datasetDimensionsRCP_;
		float v0 = texture3D(volume,  samplePos + vec3(offset.x, 0.0, 0.0)).a;
        float v1 = texture3D(volume,  samplePos + vec3(0, offset.y, 0)).a;
        float v2 = texture3D(volume,  samplePos + vec3(0, 0, offset.z)).a;
        float v3 = texture3D(volume,  samplePos + vec3(-offset.x, 0, 0)).a;
        float v4 = texture3D(volume,  samplePos + vec3(0, -offset.y, 0)).a;
        float v5 = texture3D(volume,  samplePos + vec3(0, 0, -offset.z)).a;
        gradient = vec3(v3 - v0, v4 - v1, v5 - v2);
        gradient *= volumeParameters.datasetSpacingRCP_;
        gradient *= volumeParameters.bitDepthScale_;
    }
