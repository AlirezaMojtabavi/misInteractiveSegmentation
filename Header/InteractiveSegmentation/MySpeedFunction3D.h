...
public:
	...
	typedef Superclass::ScalarValueType ScalarValue;
	typedef Superclass::FeatureScalarType FeatureScalar;
...
Superclass::ScalarValueType th, lap, max_image;

ScalarValue upper_threshold = static_cast<ScalarValue>(m_UpperThreshold);
ScalarValue lower_threshold = static_cast<ScalarValue>(m_LowerThreshold);
ScalarValue mid = ((upper_threshold - lower_threshold) / 2.0) + lower_threshold;
ScalarValue threshold;
ScalarValue   m_EdgeWeight;
FeatureScalar m_UpperThreshold;
FeatureScalar m_LowerThreshold;
...
inline void MySpeedFunction3D<TImageType, TFeatureImageType>::CalculateSpeedImage()
{
	...
	ImageCalculatorFilterType* imageCalculatorFilter1
		= ImageCalculatorFilterType::New();
	...
	{
		if (static_cast<ScalarValue>(fit.Get()) < mid)
		{
		...
		if (m_EdgeWeight != 0.0)
		{
			...
			sit.Set(static_cast<ScalarValue>(threshold + m_EdgeWeight * lit.Get()));
			lap = lit.Get();
			++lit;
		}
		else
		{
			sit.Set(static_cast<ScalarValue>(threshold));
