...
public:
	...
	bool QueryServer(DcmDataset* query, const GNC::GCS::Ptr<DicomServer>& server, IModeloDicom* pModelo, const std::string& local_aet, CallbackType callbackType) {
		...
		//SM T::SetNotificadorProgreso(m_pNotificadorProgreso);
			...
			for (unsigned long i = 0; i < result->card(); ++i) {
				...
				switch (callbackType) {
					case CT_FindPacientes:
						OnBuscarPacientes(query, dset, server, pModelo);
						break;
					case CT_FindEstudios:
						OnBuscarEstudios(query, dset, server, pModelo);
						break;
					case CT_FindSeries:
						OnBuscarSeries(query, dset, server, pModelo);
						break;
					case CT_FindImagenes:
						OnBuscarImagenes(query,dset, server, pModelo);
						break;
					case CT_MoveEstudio:
