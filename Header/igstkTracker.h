...
class Tracker : public Object, public ICoordinateSystem
{
...
public:
  ...
  std::string GetUID() const; // return a unique identifier string 
  
  void SetUID(const std::string& UID);

  // Get the coordinate system attached to the tracker camera (which might or might not be identical to the tracker default
  // coordinate system). The default implementation returns NULL which means that the default tracker does not need to 
  // use a camera. Subclasses that use a camera MUST override this method.
  virtual std::shared_ptr<const ICoordinateSystem> GetCameraCoordinateSystem() const;

  // The tracker needs a coordinate system correlation manager to update its camera transform.
  void SetCorrelationManager(std::shared_ptr< ICoordinateSystemCorrelationManager<std::string> > correlationManager);

  bool StopTracking();//Returns true if stop succeed, false if stop failed.
  bool StartTracking();
...
protected:
  ...
  *  valid. */
  ...
  // Sets the transform from the tracker camera to the tracker default coordinate system. For trackers that use a camera 
  // for getting a fix on tracker tools, the camera coordinate system might or might not be identical to the default 
  // coordinate system - it could be attached to one of the tracker tools or another coordinate system.
  void SetCameraTransform( const TransformType& transform );
  ...
  //By Parsiss: we changed this method to virtual so that we can override this in igstkMicronTracker and
  //When calling this method, we don`t terminate the data acquisition thread.
  virtual void ExitTrackingStateProcessing( void );
  ...
  // Added BY : JHB, parsiss Team
  void 	SetTrackerToolExtraInformation( TrackerToolType * trackerTool, const ToolExtraInformation extraInformation );
...
private:
  ...
  itk::MutexLock::Pointer 	m_TrackerToolsLock;
  /** typedefs from TrackerTool class */
  ...
  /** Thread function for tracking */
	//This func changed by -M.A- to enable us send event of micron tracker disconnection to misCore
  ...
  //UID is a string that specifies a unique ID for each coordinate system, so that a coordinate system could be discriminated well 
  std::string m_Uid;
  
  // saves the value whether uid has set by program or not
  bool m_IsUidSet;
...
public:
		// Mutex To Control Update Status By Start-Stop-Null
		itk::MutexLock::Pointer 	m_ControlSTNLock;
		enum UpdateStateCommand
		{
			Start=0,
			Stop,
			None
		};
		UpdateStateCommand* m_ControlSTNPointer;
		UpdateStateCommand m_ControlSTNTemp;
		itk::ConditionVariable::Pointer m_StopStartCV;
		itk::SimpleMutexLock*           m_LockForConditionStopStartReceived;
		itk::SmartPointer < misTrackingObserver >       m_TrackerObserver;
		std::shared_ptr<ITrackerConnectivity> m_TrackerConnectivity;
		protected:
			int m_Priority;

		
		public:
			//This func added by -M.A- to enable us send event of micron tracker disconnection to misCore
			ITK_THREAD_RETURN_TYPE UpdateCameraInThread(void * pInfo);		
};
