// old NAME vtkIterativeClosestPointTransform2 - Implementation of the ICP algorithm.
// New name misIterativeClosestPointTransform
...
class misIterativeClosestPointTransform : public vtkLinearTransform
{
public:
  static misIterativeClosestPointTransform *New();
  ...
  // Description:
  // Set/Get a spatial locater for speeding up the search process. 
  ...
  void GetTransformedPoints(vtkPoints* pTransformedPoints);

  double      ComputePointDistanseToSurface(double position[3]);
  double      GetFinallandmarkDistanseToSurface(void);


protected:

  void     ComputeFinalMeanPointsDistanseToSurface(void);
  vtkPoints* EliminateFarPoints( vtkPoints* initialPoints);
  void ComputeICP(vtkPoints* selectedPoints);
  // Description:
  ...
  misIterativeClosestPointTransform();
  ~misIterativeClosestPointTransform();
  ...
  vtkPoints* m_transformedPoints;
  double    m_FinalLandmarkMeanDistanseToSurface;
