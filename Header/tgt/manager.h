...
protected:
	...
	//const T* get(const std::string& filename) { return resourcesByFilename_[filename]->data_; }
...
std::string ResourceManager<T>::completePath(std::string filename) {
    ...
    if (!cplFileName.empty()) {
        ...
        while (iter != pathList_.end() && !foundFile) {
			...
			FileSystem* pointerFile=Singleton<FileSystem>::GetInstance();
            if (pointerFile->exists(cplFileName))
                foundFile = true;
            iter++;
        }
    }
