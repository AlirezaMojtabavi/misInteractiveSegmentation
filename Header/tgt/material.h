...
public:
    ...
    //! Destructor.
    ~Material() {
        if ( textureDiffuse_ ) textureMnager->dispose( textureDiffuse_ );
        if ( textureReflective_ ) textureMnager->dispose( textureReflective_ );
        if ( textureTransparent_ ) textureMnager->dispose( textureTransparent_ );
    }   
...
private:
	...
	Texture* textureTransparent_;
	 tgt::TextureManager*   textureMnager;
    };
