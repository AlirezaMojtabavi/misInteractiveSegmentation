...
#include "misRootVolumeTexture.h"
#include "misHeader.h"
 ...
 */
class MISDLLEXPORT VolumeGL {
public:

	void setFormat(GLint format){format_ = format ;}
	void setType(GLenum type){dataType_ = type ;}
  ...
  /*  const VolumeTexture* getTexture(size_t i = 0) const;
	...
	const misRootVolumeTexture* getTexture(size_t i = 0) const;

	/// @overload
	misRootVolumeTexture* getTexture(size_t i = 0);
  ...
  //  void setFilter(tgt::Texture::Filter filter);
	void setTexture( int i, misRootVolumeTexture * tex );
protected:
	/**
...
//    VolumeTextures textures_;
   ...
   typedef std::vector<misRootVolumeTexture*> VolumeTextures;
	VolumeTextures textures_;
