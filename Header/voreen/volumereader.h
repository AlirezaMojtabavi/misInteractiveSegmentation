...
public:
     ...
     */
    virtual misOpenglTexture* read(const std::string& url) throw (FileException, std::bad_alloc) = 0;
     ...
     */
    virtual misOpenglTexture* readSlices(const std::string& url, size_t firstSlice = 0, size_t lastSlice = 0)
     ...
     */
    virtual misOpenglTexture* read(const VolumeOrigin& origin)
        throw (FileException, std::bad_alloc);
