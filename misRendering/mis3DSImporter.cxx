...
static int parse_3ds_file (mis3DSImporter *importer);
static void parse_3ds (mis3DSImporter *importer, vtk3DSChunk *mainchunk);
static void parse_mdata (mis3DSImporter *importer, vtk3DSChunk *mainchunk);
static void parse_fog (mis3DSImporter *importer, vtk3DSChunk *mainchunk);
static void parse_fog_bgnd (mis3DSImporter *importer);
static void parse_mat_entry (mis3DSImporter *importer, vtk3DSChunk *mainchunk);
static char *parse_mapname (mis3DSImporter *importer, vtk3DSChunk *mainchunk);
static void parse_named_object (mis3DSImporter *importer, vtk3DSChunk *mainchunk);
static void parse_n_tri_object (mis3DSImporter *importer, vtk3DSChunk *mainchunk);
static void parse_point_array (mis3DSImporter *importer, vtk3DSMesh *mesh);
static void parse_face_array (mis3DSImporter *importer, vtk3DSMesh *mesh, vtk3DSChunk *mainchunk);
static void parse_msh_mat_group (mis3DSImporter *importer, vtk3DSMesh *mesh);
static void parse_smooth_group (mis3DSImporter *importer);
static void parse_mesh_matrix (mis3DSImporter *importer, vtk3DSMesh *mesh);
static void parse_n_direct_light (mis3DSImporter *importer, vtk3DSChunk *mainchunk);
static void parse_dl_spotlight (mis3DSImporter *importer);
static void parse_n_camera (mis3DSImporter *importer);
static void parse_colour (mis3DSImporter *importer, vtk3DSColour *colour);
static void parse_colour_f (mis3DSImporter *importer, vtk3DSColour *colour);
static void parse_colour_24 (mis3DSImporter *importer, vtk3DSColour_24 *colour);
static float parse_percentage (mis3DSImporter *importer);
static short parse_int_percentage (mis3DSImporter *importer);
static float parse_float_percentage (mis3DSImporter *importer);
static vtk3DSMaterial *update_materials (mis3DSImporter *importer, const char *new_material, int ext);
static void start_chunk (mis3DSImporter *importer, vtk3DSChunk *chunk);
static void end_chunk (mis3DSImporter *importer, vtk3DSChunk *chunk);
static byte read_byte (mis3DSImporter *importer);
static word read_word (mis3DSImporter *importer);
static dword read_dword (mis3DSImporter *importer);
static float read_float (mis3DSImporter *importer);
static void read_point (mis3DSImporter *importer, vtk3DSVector v);
static char *read_string (mis3DSImporter *importer);

mis3DSImporter::mis3DSImporter ()
{
  this->OmniList = NULL;
...
int mis3DSImporter::ImportBegin ()
{
...
void mis3DSImporter::ImportAllActor()
{
  ...
  for (mesh = this->MeshList; mesh != (vtk3DSMesh *) NULL; 
	...
	bachBlock  block;
	block.algorithm=polyStripper->GetOutputPort ();
	...
	block.prop=material->aProperty;
	m_Batches.push_back(block);
    m_Actors.push_back(actor)  ;
  }
}

vtkPolyData *mis3DSImporter::GeneratePolyData (vtk3DSMesh *mesh)
{
  int i;
  vtk3DSFace  *face;
...
static void parse_fog_bgnd(mis3DSImporter *vtkNotUsed(importer))
{
}
  ...
  do  
    ...
    if (chunk.end <= mainchunk->end) 
      ...
      switch (chunk.tag) 
        ...
        case 0xA040: mprop->shininess = 100.0f*parse_percentage(importer);
          break;
...
static void parse_smooth_group(mis3DSImporter *vtkNotUsed(importer))
{
}

static void parse_mesh_matrix(mis3DSImporter *vtkNotUsed(importer), vtk3DSMesh *vtkNotUsed(mesh))
{
...
static void parse_colour (mis3DSImporter *importer, vtk3DSColour *colour)
{
  ...
  switch (chunk.tag) 
    ...
    case 0x0011: parse_colour_24 (importer, &colour_24);
      colour->red   = colour_24.red/255.0f;
      colour->green = colour_24.green/255.0f;
      colour->blue  = colour_24.blue/255.0f;
      break;
...
static float parse_percentage(mis3DSImporter *importer)
{
  ...
  switch (chunk.tag) 
    {
    case 0x0030: percent = parse_int_percentage(importer)/100.0f;
      break;
...
void mis3DSImporter::PrintSelf(ostream& os, vtkIndent indent)
{
...
std::vector<vtkSmartPointer<vtkActor>> mis3DSImporter::getActors()
{
	return m_Actors;
}

void mis3DSImporter::ReadData()
{
	// this->Import actors, cameras, lights and properties
	this->ImportAllActor();
	this->ImportProperties (NULL);
}
void mis3DSImporter::Read ()
{
	if (this->ImportBegin ())
	{
		this->ReadData();
...
void mis3DSImporter::ImportProperties (vtkRenderer *vtkNotUsed(renderer))
{
	float amb = 0.1f, dif = 0.9f;
	...
	for (m = this->MatPropList; m != (vtk3DSMatProp *) NULL; m = (vtk3DSMatProp *) m->next)
	{
		...
		dist_white = fabs(1.0f - m->specular.red) +
			fabs(1.0f - m->specular.green) +
			fabs(1.0f - m->specular.blue);
...
std::vector<bachBlock> mis3DSImporter::getBatches()
{
	return  m_Batches;
