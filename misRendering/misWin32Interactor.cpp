...
// sees it in the friend decl in misWin32Interactor, but
// GCC needs to see the declaration beforehand. It has to do with the
// CALLBACK attribute.
VTKRENDERINGOPENGL_EXPORT LRESULT CALLBACK misHandleMessage(HWND, UINT, WPARAM, LPARAM);
VTKRENDERINGOPENGL_EXPORT LRESULT CALLBACK misHandleMessage2(HWND, UINT, WPARAM, LPARAM, class misWin32Interactor*);
...
vtkStandardNewMacro(misWin32Interactor);

void(*misWin32Interactor::ClassExitMethod)(void *) = (void(*)(void *))NULL;
void *misWin32Interactor::ClassExitMethodArg = (void *)NULL;
void(*misWin32Interactor::ClassExitMethodArgDelete)(void *) = (void(*)(void *))NULL;

//----------------------------------------------------------------------------
// Construct object so that light follows camera motion.
misWin32Interactor::misWin32Interactor()
{
	this->WindowId = 0;
...
misWin32Interactor::~misWin32Interactor()
{
	...
	if (this->WindowId && this->Enabled && this->InstallMessageProc)
	{
		...
		tmp = (vtkWin32OpenGLRenderWindow *)GetPropA(WindowId, "RenderWindow");;
...
//----------------------------------------------------------------------------
void  misWin32Interactor::StartEventLoop()
{
...
void misWin32Interactor::Initialize()
{
	vtkWin32OpenGLRenderWindow *ren;
	int *size;
...
//----------------------------------------------------------------------------
void misWin32Interactor::Enable()
{
	vtkWin32OpenGLRenderWindow *ren;
	vtkWin32OpenGLRenderWindow *tmp;
	if (this->Enabled)
	...
	if (this->InstallMessageProc)
	{
		...
		if (tmp != ren)
		{
			...
			// event handler so that it can properly
			// call this event handler if required
			CallWindowProc(this->OldProc, this->WindowId, WM_USER + 12, 24, (intptr_t)misHandleMessage);
		}
		else
		{
			vtkSetWindowLong(this->WindowId, vtkGWL_WNDPROC, (intptr_t)misHandleMessage);
		}
...
//----------------------------------------------------------------------------
void misWin32Interactor::Disable()
{
	vtkWin32OpenGLRenderWindow *tmp;
	if (!this->Enabled)
	{
...
//----------------------------------------------------------------------------
void misWin32Interactor::TerminateApp(void)
{
...
//----------------------------------------------------------------------------
int misWin32Interactor::InternalCreateTimer(int timerId, int vtkNotUsed(timerType),
...
//----------------------------------------------------------------------------
int misWin32Interactor::InternalDestroyTimer(int platformTimerId)
{
...
// Event loop handlers
//-------------------------------------------------------------
int misWin32Interactor::OnMouseMove(HWND hWnd, UINT nFlags,
	int X, int Y)
{
	if (!this->Enabled)
	{
...
//----------------------------------------------------------------------------
int misWin32Interactor::OnNCMouseMove(HWND, UINT nFlags,
	int X, int Y)
{
...
//----------------------------------------------------------------------------
int misWin32Interactor::OnMouseWheelForward(HWND, UINT nFlags,
	int X, int Y)
{
	if (!this->Enabled)
	{
...
//----------------------------------------------------------------------------
int misWin32Interactor::OnMouseWheelBackward(HWND, UINT nFlags,
	int X, int Y)
{
	if (!this->Enabled)
	{
...
//----------------------------------------------------------------------------
int misWin32Interactor::OnLButtonDown(HWND wnd, UINT nFlags,
...
//----------------------------------------------------------------------------
int misWin32Interactor::OnLButtonUp(HWND, UINT nFlags,
	int X, int Y)
{
	if (!this->Enabled)
	{
...
//----------------------------------------------------------------------------
int misWin32Interactor::OnMButtonDown(HWND wnd, UINT nFlags,
	int X, int Y, int repeat)
{
	if (!this->Enabled)
	{
...
//----------------------------------------------------------------------------
int misWin32Interactor::OnMButtonUp(HWND, UINT nFlags,
	int X, int Y)
{
	if (!this->Enabled)
	{
...
//----------------------------------------------------------------------------
int misWin32Interactor::OnRButtonDown(HWND wnd, UINT nFlags,
	int X, int Y, int repeat)
{
	if (!this->Enabled)
	{
...
//----------------------------------------------------------------------------
int misWin32Interactor::OnRButtonUp(HWND, UINT nFlags,
	int X, int Y)
{
	if (!this->Enabled)
	{
...
//----------------------------------------------------------------------------
int misWin32Interactor::OnSize(HWND, UINT, int X, int Y)
{
	this->UpdateSize(X, Y);
	if (this->Enabled)
	{
...
//----------------------------------------------------------------------------
int misWin32Interactor::OnTimer(HWND, UINT timerId)
{
	if (!this->Enabled)
	{
		return 0;
...
//----------------------------------------------------------------------------
int misWin32Interactor::OnKeyDown(HWND, UINT vCode, UINT nRepCnt, UINT nFlags)
{
	if (!this->Enabled)
	{
		return 0;
...
//----------------------------------------------------------------------------
int misWin32Interactor::OnKeyUp(HWND, UINT vCode, UINT nRepCnt, UINT nFlags)
{
	if (!this->Enabled)
	{
		return 0;
...
//----------------------------------------------------------------------------
int misWin32Interactor::OnChar(HWND, UINT nChar,
	UINT nRepCnt, UINT)
{
	if (!this->Enabled)
	{
...
//----------------------------------------------------------------------------
int misWin32Interactor::OnFocus(HWND, UINT)
{
	if (!this->Enabled)
	{
		return 0;
...
//----------------------------------------------------------------------------
int misWin32Interactor::OnKillFocus(HWND, UINT)
{
	if (!this->Enabled)
	{
		return 0;
...
LRESULT CALLBACK misHandleMessage(HWND hWnd, UINT uMsg, WPARAM wParam,
	LPARAM lParam)
{
	LRESULT res = 0;
	vtkWin32OpenGLRenderWindow *ren;
	misWin32Interactor *me = 0;

	ren = (vtkWin32OpenGLRenderWindow *)GetPropA(hWnd, "RenderWindow");;
	if (ren)
	{
		me = (misWin32Interactor *)ren->GetInteractor();
	}

	if (me && me->GetReferenceCount()>0)
	{
		me->Register(me);
		res = misHandleMessage2(hWnd, uMsg, wParam, lParam, me);
		me->UnRegister(me);
	}
...
//----------------------------------------------------------------------------
LRESULT CALLBACK misHandleMessage2(HWND hWnd, UINT uMsg, WPARAM wParam,
	LPARAM lParam,
	misWin32Interactor *me)
{
	...
	case WM_MOUSEMOVE:
		 ...
		 std::cout << "misHandleMessage2::OnMouseMove" << std::endl;
		break;
...
void
misWin32Interactor::SetClassExitMethod(void(*f)(void *), void *arg)
{
	if (f != misWin32Interactor::ClassExitMethod
		|| arg != misWin32Interactor::ClassExitMethodArg)
	{
		// delete the current arg if there is a delete method
		if ((misWin32Interactor::ClassExitMethodArg)
			&& (misWin32Interactor::ClassExitMethodArgDelete))
		{
			(*misWin32Interactor::ClassExitMethodArgDelete)
				(misWin32Interactor::ClassExitMethodArg);
		}
		misWin32Interactor::ClassExitMethod = f;
		misWin32Interactor::ClassExitMethodArg = arg;
...
void
misWin32Interactor::SetClassExitMethodArgDelete(void(*f)(void *))
{
	if (f != misWin32Interactor::ClassExitMethodArgDelete)
	{		misWin32Interactor::ClassExitMethodArgDelete = f;
...
//----------------------------------------------------------------------------
void misWin32Interactor::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void misWin32Interactor::ExitCallback()
{
