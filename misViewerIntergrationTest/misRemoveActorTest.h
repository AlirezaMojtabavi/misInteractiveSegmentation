...
public:
	...
	typedef misRemoveActorTest Self;
	...
	misRemoveActorTest()
	{
		...
		m_PusleObserver->SetCallbackFunction(this,&misRemoveActorTest::Render);
	...
	void AddSurface()
	{
		...
		misSurfaceRepresentation::Pointer  pTool=misSurfaceRepresentation::New("ljk");

		vtkSphereSource*  pshere=vtkSphereSource::New();
		
		pTool->AddPolyData(pshere->GetOutput());

		m_Viewer->AddRepresentation(pTool);
		//m_Viewer->RemoveRepresentation(pTool);
