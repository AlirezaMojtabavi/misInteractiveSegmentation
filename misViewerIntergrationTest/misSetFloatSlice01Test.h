...
public:
	...
	//==========================================================================
	misSetFloatSlice01Test(int type)
	{
		m_Type=type;
		slice=0;
		slidenumber=0;
		...
		m_PusleObserver->SetCallbackFunction(this,&misSetFloatSlice01Test::Render);
		...
		this->m_Viewer=misVolumeSlicer::New();
		this->m_Viewer->SetWindow(m_wnd);
	...
	void AddImage()
	{
		...
		pImageReader->SetDirectoryName("F:\\Dicom data\\Mohsen Samadi\\DICOM\\PA2\\ST1\\SE1");
		pImageReader->Update();
 
		plane=misGpuAxialMultiplaneRepresentation::New("");	
	...
	void Render()
	{
		...
		if (slidenumber>100)
			slidenumber=0;
		plane->SetSliceValueIn01Range(slice);
		//plane->SetSliceNumber(slidenumber);
		//plane->Update
		if (slice>1.0)  
			slice=0;
