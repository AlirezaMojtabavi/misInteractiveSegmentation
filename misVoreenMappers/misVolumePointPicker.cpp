...
#include "misPeakFinder.h"
#include "misTransFunctionBuilder.h"
#include "misVTKMattoTgtMatConvertor.h"
#include "misVoreenCanvasMapper.h"

#define    MaxOpacityForSelctPoint  0.95


vtkStandardNewMacro(misVolumePointPicker);

misVolumePointPicker::misVolumePointPicker()
{
	this->PointId = -1;
	m_SurfaceValue=200;
...
double misVolumePointPicker::IntersectWithLine(double p1[3], double p2[3], double tol, 
										 ...
										 vtkAbstractMapper3D *m)
{
	...
	misVoreenCanvasMapper* vorrenMapper = dynamic_cast<misVoreenCanvasMapper*> (m_VolumeMapper);
	if (!vorrenMapper)
		return 2;
	int ret= vorrenMapper->FindNearestPoint(p1,p2,tol,intersectedPoint);
	
	if(ret==1)
		{
			this->PickedPositions->Reset();
			this->PickedPositions->InsertNextPoint(intersectedPoint);
...
void misVolumePointPicker::Initialize()
{
	this->PointId = (-1);
	this->vtkPicker::Initialize();
}

void misVolumePointPicker::PrintSelf(ostream& os, vtkIndent indent)
{
	this->Superclass::PrintSelf(os,indent);

	os << indent << "Point Id: " << this->PointId << "\n";
}
int misVolumePointPicker::Pick(double selectionX, double selectionY, double selectionZ,
...
misVolumePointPicker::~misVolumePointPicker(void)
{
	// TODO: it crash while calling
	//if (pGrImage)
	//	pGrImage->Delete();
}
...
void misVolumePointPicker::SetVolumeMapper( vtkMapper* val )
{
	m_VolumeMapper = val;
}
